...
from augmentations import Compose, ColorJitter, RandomHorizontalFlip, RandomCrop, RandomScale
...
class LoadDataset(data.Dataset):
    ...
    def __init__(self, mode='train'):
        ...
        crop_size = tuple(int(i) for i in crop_size_1.split(' '))
        ...
        with open(os.path.join('./Datasets/Cityscapes', self.mode + '.txt'), 'r') as f:
    ...
    def __getitem__(self, index):
        image_path, label_path = self.image_labels[index].strip().split(',')
        image = Image.open(image_path).convert('RGB')  # RGB 0~255
        label = Image.open(label_path)  # 1 channel 0~37
